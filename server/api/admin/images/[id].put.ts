...
export default defineEventHandler(async event => {
    ...
    await ready(db)
    const current = await db.prepare('SELECT id FROM images WHERE id=?').bind(id).first()
    if (!current) throw createError({statusCode: 404, statusMessage: 'Image introuvable'})
    await persistImage(db, {...body, id, darkVariant: null})
