...
import { _ as __nuxt_component_1$1, e as engagementStyles } from './engagement-icons.module-BtNn5Pri.mjs';
import { defineComponent, useCssModule, ref, computed, withAsyncContext, reactive, watch, unref, withCtx, createTextVNode, openBlock, createBlock, Fragment, renderList, createVNode, toDisplayString, createCommentVNode, mergeProps, isRef, useModel, mergeModels, useId, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderClass, ssrRenderAttr, ssrRenderList, ssrInterpolate, ssrGetDirectiveProps, ssrIncludeBooleanAttr, ssrRenderStyle, ssrLooseContain, ssrLooseEqual, ssrRenderTeleport } from 'vue/server-renderer';
import { _ as _export_sfc } from './_plugin-vue_export-helper-1tPrXgE0.mjs';
import { u as useThemedImage, r as resolveImageVariant } from './useThemedImage-HZxF4Y-0.mjs';
import { c as catalog } from './catalog.module-BY5p-w8A.mjs';
import { f as useRoute, a as useRouter, n as navigateTo, g as useFetch, i as defaultSiteContent, u as useHead, _ as __nuxt_component_7, j as useRequestURL } from './server.mjs';
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.modelValue) {
        _push(`<img${ssrRenderAttr("src", __props.modelValue.content)} style="${ssrRenderStyle(unref(previewStyle))}" alt="Aper\xE7u de l\u2019image">`);
      } else {
        _push(`<div class="${ssrRenderClass(unref(styles2).placeholder)}">Aucune image</div>`);
      }
      _push(`</div><button type="button" data-demo-interactive>${ssrInterpolate(__props.modelValue ? "Changer dans la biblioth\xE8que" : "Choisir dans la biblioth\xE8que")}</button>`);
      if (__props.modelValue) {
      ...
      _push(`</div>`);
      if (__props.modelValue) {
        _push(`<div class="${ssrRenderClass(unref(styles2).dimensions)}"><label>Largeur affich\xE9e <span><input${ssrRenderAttr("value", __props.modelValue.width)}${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} type="number" min="1"> px</span></label><b aria-label="Ratio verrouill\xE9">\u{1F512} Ratio ${ssrInterpolate(__props.modelValue.naturalWidth)}:${ssrInterpolate(__props.modelValue.naturalHeight)}</b><label>Hauteur affich\xE9e <span><input${ssrRenderAttr("value", __props.modelValue.height)}${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} type="number" min="1"> px</span></label></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<small>Image r\xE9utilisable depuis la biblioth\xE8que \xB7 dimensions synchronis\xE9es avec l\u2019aper\xE7u</small>`);
      if (unref(error2)) {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(open)) {
          _push2(`<div class="${ssrRenderClass(unref(styles2).overlay)}"><section class="${ssrRenderClass(unref(styles2).library)}" role="dialog" aria-modal="true"${ssrRenderAttr("aria-label", `Biblioth\xE8que \u2014 ${__props.label}`)}><header><div><small>M\xC9DIATH\xC8QUE</small><h2>Choisir une image</h2></div><button type="button" aria-label="Fermer">\xD7</button></header><div class="${ssrRenderClass(unref(styles2).grid)}"><label class="${ssrRenderClass(unref(styles2).add)}"${ssrRenderAttr("for", unref(inputId))}><b>+</b><span>Ajouter une image</span><input${ssrRenderAttr("id", unref(inputId))} type="file"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} accept="image/jpeg,image/png,image/webp,image/gif"></label><!--[-->`);
          ssrRenderList(unref(library2), (image) => {
            var _a;
            _push2(`<article class="${ssrRenderClass(((_a = __props.modelValue) == null ? void 0 : _a.id) === image.id ? unref(styles2).selected : "")}"><button type="button"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} class="${ssrRenderClass(unref(styles2).pick)}"><span class="${ssrRenderClass(unref(styles2).pair)}"><img${ssrRenderAttr("src", image.content)} alt="Version claire">`);
            if (image.darkVariant) {
              _push2(`<img${ssrRenderAttr("src", image.darkVariant.content)} alt="Version sombre">`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</span><span>${ssrInterpolate(image.width)} \xD7 ${ssrInterpolate(image.height)} px</span></button><label class="${ssrRenderClass(unref(styles2).darkUpload)}">${ssrInterpolate(image.darkVariant ? "Remplacer la version sombre" : "\uFF0B Ajouter une version sombre")}<input type="file"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} accept="image/jpeg,image/png,image/webp,image/gif"></label><button type="button"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} class="${ssrRenderClass(unref(styles2).deleteImage)}"${ssrRenderAttr("title", image.usageCount ? `Image utilis\xE9e ${image.usageCount} fois` : "Supprimer cette image")}${ssrRenderAttr("aria-label", `Supprimer l\u2019image ${image.id}`)}>Supprimer </button></article>`);
          });
          _push2(`<!--]--></div>`);
          if (unref(loading)) {
            _push2(`<p>Chargement\u2026</p>`);
          } else if (unref(library2).length === 0) {
            _push2(`<p>Aucune image enregistr\xE9e pour le moment.</p>`);
          } else {
            _push2(`<!---->`);
          }
          if (unref(error2)) {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(topGroups, (group, index) => {
        var _a2;
        _push(`<!--[--><details${ssrIncludeBooleanAttr(index === 0) ? " open" : ""}><summary>${ssrInterpolate(group.title)}<span>${ssrInterpolate((_a2 = group.fields) == null ? void 0 : _a2.length)} champs</span></summary><div class="${ssrRenderClass(unref(editor$1).fields)}"><!--[-->`);
        ...
        _push(`<!--]--></div>`);
        if (group.title === "Banni\xE8re principale") {
      ...
      _push(ssrRenderComponent(_component_ImageUpload, {
        ...
        label: "Ic\xF4ne du premier engagement",
        readonly: __props.readonly,
        required: ""
      }, null, _parent));
      _push(`</article><article><label>Deuxi\xE8me engagement<input${ssrRenderAttr("value", model.value.value2)} maxlength="500"></label>`);
      _push(ssrRenderComponent(_component_ImageUpload, {
        modelValue: model.value.value2Image,
        "onUpdate:modelValue": ($event) => model.value.value2Image = $event,
        label: "Ic\xF4ne du deuxi\xE8me engagement",
        readonly: __props.readonly,
        required: ""
      }, null, _parent));
      _push(`</article><article><label>Troisi\xE8me engagement<input${ssrRenderAttr("value", model.value.value3)} maxlength="500"></label>`);
      _push(ssrRenderComponent(_component_ImageUpload, {
        modelValue: model.value.value3Image,
        "onUpdate:modelValue": ($event) => model.value.value3Image = $event,
        label: "Ic\xF4ne du troisi\xE8me engagement",
        readonly: __props.readonly,
        required: ""
      }, null, _parent));
      _push(`</article></div></details><details><summary>Univers<span>${ssrInterpolate((_a = universes.value) == null ? void 0 : _a.length)} univers</span></summary><div class="${ssrRenderClass(unref(editor$1).fields)}"><!--[-->`);
      ...
      ssrRenderList(universes.value, (universe, index) => {
        var _a2;
        _push(`<article><div class="${ssrRenderClass(unref(editor$1).universeToolbar)}"><strong>Univers ${ssrInterpolate(index + 1)}</strong><span><button type="button"${ssrIncludeBooleanAttr(index === 0) ? " disabled" : ""}>\u2191</button><button type="button"${ssrIncludeBooleanAttr(index === ((_a2 = universes.value) == null ? void 0 : _a2.length) - 1) ? " disabled" : ""}>\u2193</button><button type="button">Supprimer</button></span></div><label>Nom<input${ssrRenderAttr("value", universe.title)} maxlength="100" required>`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
          field: `universes.${index}.title`
        }, null, _parent));
        _push(`</label><label>Slug (facultatif)<input${ssrRenderAttr("value", universe.slug)} maxlength="100" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="L\u2019identifiant sera utilis\xE9 si vide">`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
      ...
      _push(`<!--]--><button type="button" class="${ssrRenderClass(unref(editor$1).addUniverse)}">+ Ajouter un univers </button></div></details><details><summary>S\xE9lection produits<span>${ssrInterpolate(selectionFields.length)} champs</span></summary><div class="${ssrRenderClass(unref(editor$1).fields)}"><!--[-->`);
      ...
      ssrRenderList(unref(selectedProducts), (product, index) => {
        ...
        if (unref(imageFor)(product.image)) {
          _push(`<img${ssrRenderAttr("src", ((_a2 = unref(imageFor)(product.image)) == null ? void 0 : _a2.content) + `?size=${(_b2 = unref(imageFor)(product.image)) == null ? void 0 : _b2.width}x${(_c2 = unref(imageFor)(product.image)) == null ? void 0 : _c2.height}`)}${ssrRenderAttr("alt", product.name)}>`);
        } else {
          _push(`<!---->`);
        }
      ...
      if (favoriteIds.value.length < 4) {
        _push(`<button type="button" data-demo-interactive class="${ssrRenderClass(unref(pickerStyles).add)}">+ S\xE9lectionner un produit </button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<p>${ssrInterpolate(favoriteIds.value.length)} produit${ssrInterpolate(favoriteIds.value.length > 1 ? "s" : "")} sur 4 s\xE9lectionn\xE9${ssrInterpolate(favoriteIds.value.length > 1 ? "s" : "")}</p>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: __props.validationIssues,
        field: "favoriteIds"
      }, null, _parent));
      ...
      if (previewImage(model.value.value2Image)) {
        _push(`<img${ssrRenderAttr("src", ((_f = previewImage(model.value.value2Image)) == null ? void 0 : _f.content) + `?size=${(_g = previewImage(model.value.value2Image)) == null ? void 0 : _g.width}x${(_h = previewImage(model.value.value2Image)) == null ? void 0 : _h.height}`)} alt="">`);
      } else {
        _push(`<!---->`);
      }
      ...
      if (previewImage(model.value.value3Image)) {
        _push(`<img${ssrRenderAttr("src", (_i = previewImage(model.value.value3Image)) == null ? void 0 : _i.content)} alt="">`);
      } else {
        _push(`<!---->`);
      }
      ...
      ssrRenderList(unref(selectedProducts), (product) => {
        ...
        if (previewImage(product.image)) {
          _push(`<img${ssrRenderAttr("src", ((_a2 = previewImage(product.image)) == null ? void 0 : _a2.content) + `?size=${(_b2 = previewImage(product.image)) == null ? void 0 : _b2.width}x${(_c2 = previewImage(product.image)) == null ? void 0 : _c2.height}`)}${ssrRenderAttr("alt", product.name)}>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div><div class="${ssrRenderClass(unref(styles).productInfo)}"><div><small>${ssrInterpolate(product.categories.map((category) => category.label).join(" \xB7 "))}</small><h3>${ssrInterpolate(product.name)}</h3><strong>${ssrInterpolate((product.priceCents / 100).toFixed(2).replace(".", ","))} \u20AC</strong></div><button type="button" tabindex="-1" aria-hidden="true">+</button></div></article>`);
      });
      _push(`<!--]-->`);
      if (!unref(selectedProducts).length) {
        _push(`<p class="${ssrRenderClass(unref(pickerStyles).previewEmpty)}">S\xE9lectionnez jusqu\u2019\xE0 4 produits favoris</p>`);
      } else {
        _push(`<!---->`);
      }
      ...
      if (previewImage(model.value.workshopImage)) {
        _push(`<img${ssrRenderAttr("src", ((_j = previewImage(model.value.workshopImage)) == null ? void 0 : _j.content) + `?size=${(_k = previewImage(model.value.workshopImage)) == null ? void 0 : _k.width}x${(_l = previewImage(model.value.workshopImage)) == null ? void 0 : _l.height}`)} style="${ssrRenderStyle({
          width: `${previewPixels(model.value.workshopImage, 20)}px`,
          height: `${previewPixels(model.value.workshopImage, 20) * (((_m = previewImage(model.value.workshopImage)) == null ? void 0 : _m.naturalHeight) || 1) / (((_n = previewImage(model.value.workshopImage)) == null ? void 0 : _n.naturalWidth) || 1)}px`,
          maxWidth: "100%",
          maxHeight: "100%"
        })}" alt="">`);
      } else {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(pickerOpen)) {
          _push2(`<div class="${ssrRenderClass(unref(pickerStyles).overlay)}"><section class="${ssrRenderClass(unref(pickerStyles).modal)}" role="dialog" aria-modal="true" aria-label="S\xE9lection des produits favoris"><header><div><small>FAVORIS DU MOMENT</small><h2>${ssrInterpolate(unref(replaceIndex) === null ? "S\xE9lectionner les produits" : "Choisir un produit de remplacement")}</h2><p>${ssrInterpolate(unref(replaceIndex) === null ? "Vous pouvez retenir jusqu\u2019\xE0 quatre articles." : "Le produit choisi remplacera le favori actuel.")}</p></div><button type="button" aria-label="Fermer">\xD7</button></header><div class="${ssrRenderClass(unref(pickerStyles).grid)}"><!--[-->`);
          ssrRenderList(__props.products, (product) => {
            ...
            if (unref(imageFor)(product.image)) {
              _push2(`<img${ssrRenderAttr("src", ((_a2 = unref(imageFor)(product.image)) == null ? void 0 : _a2.content) + `?size=${(_b2 = unref(imageFor)(product.image)) == null ? void 0 : _b2.width}x${(_c2 = unref(imageFor)(product.image)) == null ? void 0 : _c2.height}`)}${ssrRenderAttr("alt", product.name)}>`);
            } else {
              _push2(`<!---->`);
            }
          ...
          _push2(`<!--]--></div><footer class="${ssrRenderClass(_ctx.$style.footerMain)}"><span>${ssrInterpolate(favoriteIds.value.length)} / 4 s\xE9lectionn\xE9s</span><button type="button">Terminer</button></footer></section></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: unref(editor$1).layout
      }, _attrs))}><div class="${ssrRenderClass([unref(editor$1).controls, _ctx.$style.controls])}"><details open style="${ssrRenderStyle({ "cursor": "default" })}"><summary>Contenu g\xE9n\xE9rique <span>PAGE CAT\xC9GORIE</span></summary><div class="${ssrRenderClass(unref(editor$1).fields)}"><label class="${ssrRenderClass(unref(page$1).full)}">Titre de la page<input value="Nom de la cat\xE9gorie (dynamique)" disabled><small>Le titre reprend automatiquement le nom de la cat\xE9gorie consult\xE9e.</small></label><label>Surtitre<input${ssrRenderAttr("value", model.value.categoryEyebrow)}>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: __props.validationIssues,
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: unref(editor$1).layout
      }, _attrs))}><div class="${ssrRenderClass([unref(editor$1).controls, _ctx.$style.controls])}"><details open style="${ssrRenderStyle({ "cursor": "default" })}"><summary>Contenu g\xE9n\xE9rique <span>PAGE UNIVERS</span></summary><div class="${ssrRenderClass(unref(editor$1).fields)}"><label class="${ssrRenderClass(unref(page$1).full)}">Titre de la page<input value="Nom de l\u2019univers (dynamique)" disabled><small>Le titre reprend automatiquement le nom de l\u2019univers consult\xE9.</small></label><label>Surtitre<input${ssrRenderAttr("value", model.value.universeEyebrow)}>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: __props.validationIssues,
        field: "universeEyebrow"
      }, null, _parent));
      _push(`</label><label>Libell\xE9 du filtre global<input${ssrRenderAttr("value", model.value.universeAllLabel)}>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: __props.validationIssues,
...
const facebookIcon = "" + buildAssetsURL("facebook.C8eXrwbB.png");
const instagramIcon = "" + buildAssetsURL("instagram.D-uA-gcM.png");
const whatsappIcon = "" + buildAssetsURL("whatsapp.COYr-lUP.png");
const xIcon$2 = "" + buildAssetsURL("x.B2tJsFmU.png");
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const definitions = {
      product: {
        ...
        variables: {
          ...
          "Description du produit": "Un t-shirt en coton doux, imprim\xE9 \xE0 la demande en France.",
          "Prix": "29,90 \u20AC",
          ["Cat\xE9gories"]: "V\xEAtements",
          "Univers": "Manga & Japon",
      ...
      category: {
        ...
        variables: { ["Nom de la cat\xE9gorie"]: "V\xEAtements", "Nom du site": "Angel Dreamer" }
      },
      universeCategory: {
        label: "Cat\xE9gorie d\u2019univers",
        ...
        variables: {
          ...
          ["Nom de la cat\xE9gorie"]: "V\xEAtements",
          "Nom du site": "Angel Dreamer"
        }
      }
    };
    ...
    const value = (key) => {
      var _a;
      return String((_a = content.value[key]) != null ? _a : "");
    };
    ...
    const socialPreviewImageUrl = computed(() => {
      var _a, _b;
      if (props.page === "product" && productUsesLibraryImage.value) return ((_a = content.value.seoProductOgImage) == null ? void 0 : _a.content) || "/og.png";
      return ((_b = content.value.seoOgImage) == null ? void 0 : _b.content) || "/og.png";
    });
    return (_ctx, _push, _parent, _attrs) => {
      const _component_ImageUpload = __nuxt_component_10;
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: _ctx.$style.layout
      }, _attrs))}><section class="${ssrRenderClass(_ctx.$style.editor)}"><header><small>MOD\xC8LE DYNAMIQUE</small><h3>${ssrInterpolate(unref(definition).label)}</h3><p>Cliquez sur les informations \xE0 ins\xE9rer. Elles seront remplac\xE9es automatiquement pour chaque page.</p></header><!--[-->`);
      ssrRenderList([
        ...
        _push(`<article class="${ssrRenderClass(_ctx.$style.field)}"><label><strong>${ssrInterpolate(field2.label)}</strong><span class="${ssrRenderClass(replace(value(field2.key)).length > field2.limit ? _ctx.$style.over : "")}">${ssrInterpolate(field2.limit - replace(value(field2.key)).length)} caract\xE8res restants dans l\u2019exemple</span></label><textarea${ssrRenderAttr("rows", field2.rows)}>${ssrInterpolate(value(field2.key))}</textarea><div class="${ssrRenderClass(_ctx.$style.variables)}"><small>Ins\xE9rer :</small><!--[-->`);
      ...
      _push(`</section><aside class="${ssrRenderClass(_ctx.$style.preview)}"><small>EXEMPLE R\xC9EL</small><h3>${ssrInterpolate(unref(definition).label)}</h3><section class="${ssrRenderClass(_ctx.$style.google)}"><small>${ssrInterpolate((unref(content).seoCanonicalUrl || "https://votre-boutique.fr") + unref(definition).path)}</small><strong>${ssrInterpolate(unref(renderedTitle))}</strong><p>${ssrInterpolate(unref(renderedDescription))}</p></section><div class="${ssrRenderClass(_ctx.$style.socialPreviewHead)}"><small>APER\xC7U SOCIAL</small><div class="${ssrRenderClass(_ctx.$style.previewSelector)}"><h3>${ssrInterpolate(unref(preview2) === "x" ? "X (Twitter)" : unref(preview2)[0].toUpperCase() + unref(preview2).slice(1))}</h3><div class="${ssrRenderClass(_ctx.$style.switcher)}"><!--[-->`);
      ssrRenderList(["facebook", "x", "whatsapp", "instagram"], (network) => {
        _push(`<button type="button" data-demo-interactive${ssrRenderAttr("aria-label", `Aper\xE7u ${network}`)}${ssrRenderAttr("title", network)}${ssrRenderAttr("aria-pressed", unref(preview2) === network)}><img${ssrRenderAttr("src", socialIcons[network])} class="${ssrRenderClass(network === "x" && _ctx.$style.xIcon)}" alt="" aria-hidden="true"></button>`);
      });
      _push(`<!--]--></div></div></div><section class="${ssrRenderClass([_ctx.$style.social, _ctx.$style[unref(preview2)]])}"><img${ssrRenderAttr("src", unref(socialPreviewImageUrl))} alt="Aper\xE7u de l\u2019image sociale"><div><small>${ssrInterpolate(unref(content).seoSiteName)}</small><strong>${ssrInterpolate(unref(renderedOgTitle))}</strong><p>${ssrInterpolate(unref(renderedOgDescription))}</p></div></section>`);
      if (unref(preview2) === "instagram") {
        _push(`<p class="${ssrRenderClass(_ctx.$style.platformNote)}">Instagram n\u2019affiche pas de carte dans les l\xE9gendes ; cet aper\xE7u correspond au partage du lien en message priv\xE9.</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<p class="${ssrRenderClass(_ctx.$style.note)}">${ssrInterpolate(__props.page === "product" ? unref(productUsesLibraryImage) ? "L\u2019image s\xE9lectionn\xE9e dans la m\xE9diath\xE8que sera utilis\xE9e pour tous les produits." : "L\u2019image propre \xE0 chaque produit sera utilis\xE9e automatiquement." : unref(definition).image ? "L\u2019image propre \xE0 l\u2019univers sera utilis\xE9e en priorit\xE9." : "L\u2019image Open Graph g\xE9n\xE9rale sera utilis\xE9e pour les cat\xE9gories.")}</p></aside></div>`);
    };
  }
});
...
const adminPageTabs = [
  ...
  { id: "category", label: "Page cat\xE9gorie", slug: "categorie" },
...
const adminSeoTabs = [
  ...
  { id: "universeCategory", label: "Page cat\xE9gories d\u2019univers", slug: "categories-univers" },
  { id: "category", label: "Pages cat\xE9gorie", slug: "categories" }
];
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const imageUrl = computed(() => {
      var _a;
      return ((_a = content.value.seoOgImage) == null ? void 0 : _a.content) || "/og.png";
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_AdminSubTabs, {
        ...
        label: "les sous-onglets de r\xE9f\xE9rencement",
        "id-prefix": "seo-tab",
      ...
      if (__props.section !== "home") {
        ...
        _push(`<div class="${ssrRenderClass(_ctx.$style.layout)}"><div class="${ssrRenderClass(_ctx.$style.fields)}"><details open><summary>R\xE9f\xE9rencement g\xE9n\xE9ral</summary><div class="${ssrRenderClass(_ctx.$style.grid)}"><label>Nom du site <span>${ssrInterpolate(70 - unref(content).seoSiteName.length)} caract\xE8res restants</span><input${ssrRenderAttr("value", unref(content).seoSiteName)} maxlength="70"></label><label>Titre principal <span>${ssrInterpolate(60 - unref(content).seoTitle.length)} caract\xE8res restants</span><input${ssrRenderAttr("value", unref(content).seoTitle)} maxlength="60">`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
          field: "seoTitle"
        }, null, _parent));
        _push(`</label><label class="${ssrRenderClass(_ctx.$style.wide)}">Meta description <span>${ssrInterpolate(160 - unref(content).seoDescription.length)} caract\xE8res restants</span><textarea maxlength="160" rows="3">${ssrInterpolate(unref(content).seoDescription)}</textarea>`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
          field: "seoDescription"
        }, null, _parent));
        _push(`</label><label class="${ssrRenderClass(_ctx.$style.wide)}">Mots-cl\xE9s<textarea rows="2" placeholder="mot-cl\xE9, autre mot-cl\xE9">${ssrInterpolate(unref(content).seoKeywords)}</textarea></label><label>URL canonique<input${ssrRenderAttr("value", unref(content).seoCanonicalUrl)} type="url" placeholder="https://example.com">`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
          field: "seoCanonicalUrl"
        }, null, _parent));
        _push(`</label><label>Directives robots<select><option${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoRobots) ? ssrLooseContain(unref(content).seoRobots, null) : ssrLooseEqual(unref(content).seoRobots, null)) ? " selected" : ""}>index, follow, max-image-preview:large</option><option${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoRobots) ? ssrLooseContain(unref(content).seoRobots, null) : ssrLooseEqual(unref(content).seoRobots, null)) ? " selected" : ""}>index, nofollow</option><option${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoRobots) ? ssrLooseContain(unref(content).seoRobots, null) : ssrLooseEqual(unref(content).seoRobots, null)) ? " selected" : ""}>noindex, follow</option><option${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoRobots) ? ssrLooseContain(unref(content).seoRobots, null) : ssrLooseEqual(unref(content).seoRobots, null)) ? " selected" : ""}>noindex, nofollow</option></select></label><label>Auteur<input${ssrRenderAttr("value", unref(content).seoAuthor)}></label><label>Langue<input${ssrRenderAttr("value", unref(content).seoLanguage)} placeholder="fr"></label><label>Couleur du navigateur<input${ssrRenderAttr("value", unref(content).seoThemeColor)} type="color"></label></div></details><details><summary>Open Graph et r\xE9seaux sociaux</summary><div class="${ssrRenderClass(_ctx.$style.grid)}"><label>Titre Open Graph <span>${ssrInterpolate(60 - unref(content).seoOgTitle.length)} caract\xE8res restants</span><input${ssrRenderAttr("value", unref(content).seoOgTitle)} maxlength="60"></label><label>Type<select><option value="website"${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoOgType) ? ssrLooseContain(unref(content).seoOgType, "website") : ssrLooseEqual(unref(content).seoOgType, "website")) ? " selected" : ""}>Site web</option><option value="product.group"${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoOgType) ? ssrLooseContain(unref(content).seoOgType, "product.group") : ssrLooseEqual(unref(content).seoOgType, "product.group")) ? " selected" : ""}>Catalogue produit</option><option value="business.business"${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoOgType) ? ssrLooseContain(unref(content).seoOgType, "business.business") : ssrLooseEqual(unref(content).seoOgType, "business.business")) ? " selected" : ""}>Entreprise</option></select></label><label class="${ssrRenderClass(_ctx.$style.wide)}">Description Open Graph <span>${ssrInterpolate(160 - unref(content).seoOgDescription.length)} caract\xE8res restants</span><textarea maxlength="160" rows="3">${ssrInterpolate(unref(content).seoOgDescription)}</textarea></label><label>Locale<input${ssrRenderAttr("value", unref(content).seoOgLocale)} placeholder="fr_FR"></label>`);
        _push(ssrRenderComponent(_component_ImageUpload, {
          modelValue: unref(content).seoOgImage,
          "onUpdate:modelValue": ($event) => unref(content).seoOgImage = $event,
          class: _ctx.$style.wide,
          readonly: __props.readonly,
          label: "Image sociale (1200 \xD7 630 px recommand\xE9)"
        }, null, _parent));
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: __props.validationIssues,
          field: "seoOgImage"
        }, null, _parent));
        _push(`<label>Format de carte X<select><option value="summary_large_image"${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoTwitterCard) ? ssrLooseContain(unref(content).seoTwitterCard, "summary_large_image") : ssrLooseEqual(unref(content).seoTwitterCard, "summary_large_image")) ? " selected" : ""}>Grande image</option><option value="summary"${ssrIncludeBooleanAttr(Array.isArray(unref(content).seoTwitterCard) ? ssrLooseContain(unref(content).seoTwitterCard, "summary") : ssrLooseEqual(unref(content).seoTwitterCard, "summary")) ? " selected" : ""}>R\xE9sum\xE9 compact</option></select></label><label>Compte X du site<input${ssrRenderAttr("value", unref(content).seoTwitterSite)} placeholder="@angel_dreamer"></label><label>Cr\xE9ateur X<input${ssrRenderAttr("value", unref(content).seoTwitterCreator)} placeholder="@createur"></label><label>Titre X <span>${ssrInterpolate(70 - unref(content).seoTwitterTitle.length)} caract\xE8res restants</span><input${ssrRenderAttr("value", unref(content).seoTwitterTitle)} maxlength="70"></label><label class="${ssrRenderClass(_ctx.$style.wide)}">Description X <span>${ssrInterpolate(200 - unref(content).seoTwitterDescription.length)} caract\xE8res restants</span><textarea maxlength="200" rows="2">${ssrInterpolate(unref(content).seoTwitterDescription)}</textarea></label></div></details><details><summary>Organisation et donn\xE9es structur\xE9es</summary><div class="${ssrRenderClass(_ctx.$style.grid)}"><label>Nom public<input${ssrRenderAttr("value", unref(content).seoOrganizationName)}></label><label>Raison sociale<input${ssrRenderAttr("value", unref(content).seoOrganizationLegalName)}></label><label>Site officiel<input${ssrRenderAttr("value", unref(content).seoOrganizationUrl)} type="url"></label><label>Email<input${ssrRenderAttr("value", unref(content).seoOrganizationEmail)} type="email"></label><label>T\xE9l\xE9phone<input${ssrRenderAttr("value", unref(content).seoOrganizationPhone)} type="tel"></label><label>Pays (ISO)<input${ssrRenderAttr("value", unref(content).seoOrganizationCountry)} maxlength="2"></label></div></details><details><summary>Outils pour webmasters</summary><div class="${ssrRenderClass(_ctx.$style.grid)}"><label>Validation Google Search Console<input${ssrRenderAttr("value", unref(content).seoGoogleVerification)}></label><label>Validation Bing Webmaster Tools<input${ssrRenderAttr("value", unref(content).seoBingVerification)}></label></div></details></div><aside class="${ssrRenderClass(_ctx.$style.preview)}"><div class="${ssrRenderClass(_ctx.$style.googlePreviewHead)}"><small>APER\xC7U GOOGLE</small><h3>R\xE9sultat de recherche</h3></div><article class="${ssrRenderClass(_ctx.$style.googlePreview)}"><small>${ssrInterpolate(unref(displayUrl))}</small><strong>${ssrInterpolate(unref(content).seoTitle)}</strong><p>${ssrInterpolate(unref(content).seoDescription)}</p></article><div class="${ssrRenderClass(_ctx.$style.previewHead)}"><small>APER\xC7U SOCIAL</small><div class="${ssrRenderClass(_ctx.$style.previewSelector)}"><h3>${ssrInterpolate(unref(preview2) === "x" ? "X (Twitter)" : unref(preview2)[0].toUpperCase() + unref(preview2).slice(1))}</h3><div class="${ssrRenderClass(_ctx.$style.switcher)}"><!--[-->`);
        ssrRenderList(["facebook", "x", "whatsapp", "instagram"], (network) => {
          _push(`<button type="button" data-demo-interactive${ssrRenderAttr("aria-label", `Aper\xE7u ${network}`)}${ssrRenderAttr("title", network)}${ssrRenderAttr("aria-pressed", unref(preview2) === network)}><img${ssrRenderAttr("src", socialIcons[network])} class="${ssrRenderClass(network === "x" && _ctx.$style.xIcon)}" alt="" aria-hidden="true"></button>`);
        });
        _push(`<!--]--></div></div></div><article class="${ssrRenderClass([_ctx.$style.card, _ctx.$style[unref(preview2)]])}"><img${ssrRenderAttr("src", unref(imageUrl))} alt="Aper\xE7u Open Graph"><div><small>${ssrInterpolate(unref(displayUrl).replace(/^https?:\/\//, ""))}</small><strong>${ssrInterpolate(unref(preview2) === "x" ? unref(content).seoTwitterTitle || unref(content).seoOgTitle : unref(content).seoOgTitle)}</strong><p>${ssrInterpolate(unref(preview2) === "x" ? unref(content).seoTwitterDescription || unref(content).seoOgDescription : unref(content).seoOgDescription)}</p></div></article><p class="${ssrRenderClass(_ctx.$style.hint)}">Instagram n\u2019affiche pas de carte dans les l\xE9gendes ; cet aper\xE7u correspond au partage du lien en message priv\xE9.</p><section class="${ssrRenderClass(_ctx.$style.audit)}"><header><div><small>AUDIT INTERNE</small><h3>Score SEO</h3></div>`);
        if (unref(audit2)) {
          _push(`<strong class="${ssrRenderClass(unref(audit2).score >= 80 ? _ctx.$style.good : unref(audit2).score >= 50 ? _ctx.$style.medium : _ctx.$style.bad)}">${ssrInterpolate(unref(audit2).score)}/100 \xB7 ${ssrInterpolate(unref(audit2).grade)}</strong>`);
        } else {
          _push(`<!---->`);
        }
...
const empty = (value) => !String(value != null ? value : "").trim();
...
function validateHomeContent(content, categories, universes, favoriteIds) {
  ...
  if (!content.heroImage) issues.push(issue("heroImage", "warning", "Une image principale am\xE9liore fortement la pr\xE9sentation de la page."));
  if (String(content.heroTitle).trim().length < 20) issues.push(issue("heroTitle", "info", "Un titre plus descriptif aide les visiteurs \xE0 comprendre imm\xE9diatement l\u2019offre."));
  ...
  categories.forEach((category, index) => {
    ...
    if (categorySlugs.has(category.slug)) issues.push(issue(`categories.${index}.slug`, "error", "Ce slug est d\xE9j\xE0 utilis\xE9."));
  ...
  universes.forEach((universe, index) => {
    ...
    if (universe.slug && universeSlugs.has(universe.slug)) issues.push(issue(`universes.${index}.slug`, "error", "Ce slug est d\xE9j\xE0 utilis\xE9."));
  ...
  if (!categories.length) issues.push(issue("categories", "error", "Ajoutez au moins une cat\xE9gorie de navigation."));
  if (!favoriteIds.length) issues.push(issue("favoriteIds", "info", "Aucun favori ne sera affich\xE9 sur la page d\u2019accueil."));
  if (favoriteIds.length > 4) issues.push(issue("favoriteIds", "error", "La s\xE9lection est limit\xE9e \xE0 quatre produits."));
  return issues;
}
function validateCategoryPage(content) {
  ...
  if (String(content.categoryDescription).trim().length < 60) issues.push(issue("categoryDescription", "info", "Une introduction d\u2019au moins 60 caract\xE8res est g\xE9n\xE9ralement plus utile."));
  if (empty(content.categoryEmptyText)) issues.push(issue("categoryEmptyText", "error", "Pr\xE9cisez le message affich\xE9 lorsqu\u2019aucun produit n\u2019est disponible."));
  return issues;
}
function validateUniversePage(content) {
  ...
  if (empty(content.universeAllLabel)) issues.push(issue("universeAllLabel", "error", "Le libell\xE9 du filtre global est obligatoire."));
  if (empty(content.universeEmptyText)) issues.push(issue("universeEmptyText", "error", "Pr\xE9cisez le message affich\xE9 lorsqu\u2019aucun produit ne correspond."));
  return issues;
}
function validateSeo(content) {
  ...
  else if (title.length < 30) issues.push(issue("seoTitle", "warning", "Un titre entre 30 et 60 caract\xE8res est recommand\xE9."));
  if (title.length > 60) issues.push(issue("seoTitle", "warning", "Le titre risque d\u2019\xEAtre tronqu\xE9 au-del\xE0 de 60 caract\xE8res."));
  if (!description) issues.push(issue("seoDescription", "error", "La meta description est obligatoire."));
  else if (description.length < 120) issues.push(issue("seoDescription", "warning", "Visez 120 \xE0 160 caract\xE8res pour une description plus informative."));
  if (description.length > 160) issues.push(issue("seoDescription", "error", "La meta description ne doit pas d\xE9passer 160 caract\xE8res."));
  ...
  } else issues.push(issue("seoCanonicalUrl", "info", "Sans URL canonique explicite, l\u2019adresse courante sera utilis\xE9e."));
...
function validateProduct(product) {
  ...
  if (!Number.isInteger(product.priceCents) || product.priceCents < 1) issues.push(issue("product.priceCents", "error", "Le prix doit \xEAtre un nombre entier sup\xE9rieur \xE0 z\xE9ro."));
  if (!product.categoryIds.length) issues.push(issue("product.categoryIds", "error", "S\xE9lectionnez au moins une cat\xE9gorie."));
  if (!product.universeIds.length) issues.push(issue("product.universeIds", "error", "S\xE9lectionnez au moins un univers."));
  if (empty(product.description)) issues.push(issue("product.description", "error", "La description est obligatoire."));
  else if (product.description.trim().length < 80) issues.push(issue("product.description", "warning", "Une description plus d\xE9taill\xE9e rassurera davantage les acheteurs."));
  if (!product.image) issues.push(issue("product.image", "warning", "Le produit sera enregistr\xE9 sans image."));
  if (!product.active) issues.push(issue("product.active", "info", "Le produit restera masqu\xE9 dans la boutique."));
  return issues;
}
const mediaIcon = "" + buildAssetsURL("image.CiSJumt6.png");
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const lightFileName = computed(() => {
      var _a;
      return ((_a = lightFile.value) == null ? void 0 : _a.name) || "Choisir la version claire";
    });
    const darkFileName = computed(() => {
      var _a;
      return ((_a = darkFile.value) == null ? void 0 : _a.name) || "Choisir une alternative sombre";
    });
    return (_ctx, _push, _parent, _attrs) => {
      const _component_FieldValidation = __nuxt_component_9;
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: _ctx.$style.overlay
      }, _attrs))}><section class="${ssrRenderClass(_ctx.$style.modal)}" role="dialog" aria-modal="true" aria-labelledby="media-title"><header><div><small>ADMINISTRATION DES IMAGES</small><h2 id="media-title">M\xE9diath\xE8que</h2></div><button type="button" aria-label="Fermer">\xD7</button></header><form class="${ssrRenderClass(_ctx.$style.upload)}" novalidate><h3>Ajouter une image et ses alternatives</h3>`);
      if (props.readonly) {
        _push(`<p class="${ssrRenderClass(_ctx.$style.readOnly)}">Compte de d\xE9monstration : m\xE9diath\xE8que disponible en consultation uniquement.</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="${ssrRenderClass(_ctx.$style.fileField)}"><div class="${ssrRenderClass(_ctx.$style.fileHeading)}"><strong>Version claire</strong><span>Obligatoire</span></div><label class="${ssrRenderClass([_ctx.$style.dropzone, unref(lightFile) && _ctx.$style.selected, unref(dragging2).light && _ctx.$style.dragging])}"><input type="file"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} accept="image/jpeg,image/png,image/webp,image/gif"><img${ssrRenderAttr("src", unref(mediaIcon))} alt=""><b>${ssrInterpolate(unref(dragging2).light ? "D\xE9posez la version claire ici" : unref(lightFileName))}</b><small>${ssrInterpolate(unref(lightFile) ? `${(unref(lightFile).size / 1024).toFixed(0)} Ko` : "Cliquez ou glissez une image \xB7 JPG, PNG, WebP ou GIF \xB7 2 Mo maximum")}</small></label>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: unref(issues),
        field: "media.light"
      }, null, _parent));
      _push(`</div><div class="${ssrRenderClass(_ctx.$style.fileField)}"><div class="${ssrRenderClass(_ctx.$style.fileHeading)}"><strong>Version sombre</strong><span>Optionnelle</span></div><label class="${ssrRenderClass([_ctx.$style.dropzone, unref(darkFile) && _ctx.$style.selected, unref(dragging2).dark && _ctx.$style.dragging])}"><input type="file"${ssrIncludeBooleanAttr(__props.readonly) ? " disabled" : ""} accept="image/jpeg,image/png,image/webp,image/gif"><img${ssrRenderAttr("src", unref(mediaIcon))} alt=""><b>${ssrInterpolate(unref(dragging2).dark ? "D\xE9posez l\u2019alternative sombre ici" : unref(darkFileName))}</b><small>${ssrInterpolate(unref(darkFile) ? `${(unref(darkFile).size / 1024).toFixed(0)} Ko` : "Cliquez ou glissez une image \xB7 utilis\xE9e automatiquement en th\xE8me sombre")}</small></label>`);
      _push(ssrRenderComponent(_component_FieldValidation, {
        issues: unref(issues),
        field: "media.dark"
      }, null, _parent));
      _push(`</div><button type="submit"${ssrIncludeBooleanAttr(unref(loading) || __props.readonly) ? " disabled" : ""}>${ssrInterpolate(unref(loading) ? "Envoi\u2026" : "Ajouter \xE0 la m\xE9diath\xE8que")}</button></form><div class="${ssrRenderClass(_ctx.$style.grid)}"><!--[-->`);
      ssrRenderList(unref(library2), (image) => {
      ...
      if (unref(loading)) {
        ...
        _push(`<p>Aucune image enregistr\xE9e.</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</section></div>`);
...
const trashIcon = "" + buildAssetsURL("trash.Dou1gWBt.png");
const copyIcon = "" + buildAssetsURL("copy.CN-86aZa.png");
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const adminRoute = computed(() => {
      var _a;
      return resolveAdminRoute((_a = route.params.path) != null ? _a : route.params.tab, route.query.tab);
    });
    ...
    const isDemo = computed(() => {
      var _a;
      return Boolean((_a = me.value) == null ? void 0 : _a.readOnly);
    });
    ...
    const vDemoReadonly = {
      ...
      unmounted(element) {
        var _a;
        (_a = element.demoObserver) == null ? void 0 : _a.disconnect();
      }
    };
    const products = ref([]);
    ...
    const adminDocumentTitle = computed(() => {
      var _a, _b, _c;
      const section = mediaOpen.value ? "M\xE9diath\xE8que" : activeMainTab.value === "pages" ? (_a = adminPageTabs.find((tab) => tab.id === activePageTab.value)) == null ? void 0 : _a.label : activeMainTab.value === "seo" ? `${(_b = adminSeoTabs.find((tab) => tab.id === activeSeoSection.value)) == null ? void 0 : _b.label} \u2014 R\xE9f\xE9rencement` : (_c = tabs.value.find((tab) => tab.id === activeMainTab.value)) == null ? void 0 : _c.label;
    ...
    const discountValueEuros = computed({
      get: () => {
        var _a, _b;
        return discountDraft.value && discountDraft.value.type === "fixed" ? discountDraft.value.value / 100 : (_b = (_a = discountDraft.value) == null ? void 0 : _a.value) != null ? _b : 0;
      },
      set: (v) => {
    ...
    function scopeLabel(scope, targetId) {
      ...
      if (scope === "category") {
        ...
        return c ? `Cat\xE9gorie : ${c.label}` : `Cat\xE9gorie #${targetId}`;
      }
      if (scope === "universe") {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div>`);
      if (!((_a = unref(me)) == null ? void 0 : _a.allowed)) {
        _push(`<div class="${ssrRenderClass(unref(styles).adminNotice)}"><h2>Acc\xE8s prot\xE9g\xE9</h2><p>Connectez-vous avec un compte disposant du r\xF4le administrateur.</p>`);
        ...
        _push(`<!--[-->`);
        if (unref(isDemo)) {
          _push(`<p class="${ssrRenderClass(unref(actionStyles).demoNotice)}">Mode d\xE9monstration \u2014 toutes les zones administratives sont accessibles en lecture seule.</p>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (unref(activeMainTab) === "pages") {
          ...
          if (unref(activeTab) === "content") {
            _push(`<section id="panel-content" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-content"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>PAGE D\u2019ACCUEIL</small><h3>\xC9diteur visuel</h3><p>Contr\xF4lez les textes, la navigation, les univers et leurs images avec un aper\xE7u imm\xE9diat.</p></header>`);
            ...
            _push(`<section id="panel-category" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-category"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>PAGE CAT\xC9GORIE</small><h3>\xC9diteur visuel</h3><p>Personnalisez les contenus g\xE9n\xE9riques de toutes les pages cat\xE9gorie. Leur titre reste d\xE9termin\xE9 par la cat\xE9gorie consult\xE9e.</p></header>`);
            ...
            _push(`<section id="panel-universe" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-universe"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>PAGE UNIVERS</small><h3>\xC9diteur visuel</h3><p>Personnalisez les libell\xE9s communs \xE0 toutes les pages univers. Le titre reprend automatiquement l\u2019univers consult\xE9.</p></header>`);
            ...
            _push(`<section id="panel-cart" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-cart"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>VOTRE PANIER</small><h3>Page panier</h3><p>Configurez le titre affich\xE9 dans l\u2019onglet du navigateur pour la page panier.</p></header><label>Mod\xE8le du titre<input${ssrRenderAttr("value", unref(content).seoCartTitle)} placeholder="Votre panier | [Nom du site]"></label><div class="${ssrRenderClass(unref(actionStyles).variables)}"><small>Ins\xE9rer :</small><button type="button">+ Nom du site</button></div><div class="${ssrRenderClass(unref(actionStyles).previewTitle)}">Aper\xE7u : ${ssrInterpolate(unref(renderSeoTemplate)(unref(content).seoCartTitle, { "Nom du site": unref(content).seoSiteName }))}</div><div class="${ssrRenderClass(unref(actionStyles).bottomSave)}"><button type="button"${ssrIncludeBooleanAttr(unref(saving) === "cart") ? " disabled" : ""}>${ssrInterpolate(unref(saving) === "cart" ? "Enregistrement\u2026" : "Enregistrer la page")}</button></div></section>`);
          } else if (unref(activeTab) === "profile") {
            _push(`<section id="panel-profile" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-profile"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>MON COMPTE</small><h3>Page profil</h3><p>Personnalisez le titre de la page de gestion du compte client.</p></header><label>Mod\xE8le du titre<input${ssrRenderAttr("value", unref(content).seoProfileTitle)} placeholder="Mon compte | [Nom du site]"></label><div class="${ssrRenderClass(unref(actionStyles).variables)}"><small>Ins\xE9rer :</small><button type="button">+ Nom du site</button><button type="button">+ Pr\xE9nom</button><button type="button">+ Nom</button><button type="button">+ Email</button></div><div class="${ssrRenderClass(unref(actionStyles).previewTitle)}">Aper\xE7u : ${ssrInterpolate(unref(renderSeoTemplate)(unref(content).seoProfileTitle, {
            ...
            _push(`<section id="panel-cgu" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-cgu"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>MENTIONS L\xC9GALES</small><h3>CGU</h3><p>R\xE9digez les Conditions G\xE9n\xE9rales d&#39;Utilisation. Le lien appara\xEEtra dans le footer si le contenu n&#39;est pas vide.</p></header>`);
            ...
            _push(`<section id="panel-cgv" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-cgv"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>MENTIONS L\xC9GALES</small><h3>CGV</h3><p>R\xE9digez les Conditions G\xE9n\xE9rales de Vente. Le lien appara\xEEtra dans le footer si le contenu n&#39;est pas vide.</p></header>`);
            _push(ssrRenderComponent(_component_ClientOnly, null, {}, _parent));
            ...
            _push(`<section id="panel-contact" class="${ssrRenderClass(unref(actionStyles).pagePanel)}" role="tabpanel" aria-labelledby="page-tab-contact"><header class="${ssrRenderClass(unref(actionStyles).pageEditorHeader)}"><small>FORMULAIRE DE CONTACT</small><h3>Page contact</h3><p>Configurez l&#39;adresse e-mail qui recevra les messages envoy\xE9s via le formulaire de contact.</p></header><label class="${ssrRenderClass(unref(styles).panelHeading)}"> Email de contact <input${ssrRenderAttr("value", unref(content).contactEmail)} type="email" placeholder="contact@example.com"></label><div class="${ssrRenderClass(unref(actionStyles).bottomSave)}"><button type="button"${ssrIncludeBooleanAttr(unref(saving) === "contact") ? " disabled" : ""}>${ssrInterpolate(unref(saving) === "contact" ? "Enregistrement\u2026" : "Enregistrer")}</button></div></section>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</section>`);
        } else if (unref(activeTab) === "seo") {
          _push(`<section id="panel-seo" class="${ssrRenderClass(unref(styles).adminPanel)}" role="tabpanel" aria-labelledby="tab-seo"><div class="${ssrRenderClass([unref(styles).panelTitle, unref(actionStyles).responsivePanelTitle])}"><div><small>VISIBILIT\xC9 ET PARTAGE</small><h2>R\xE9f\xE9rencement</h2><p>Configurez les moteurs de recherche, les aper\xE7us sociaux et les donn\xE9es structur\xE9es du site.</p></div><div class="${ssrRenderClass(unref(actionStyles).actions)}"><button type="button"${ssrIncludeBooleanAttr(unref(saving) === "seo") ? " disabled" : ""}>${ssrInterpolate(unref(saving) === "seo" ? "Enregistrement\u2026" : "Enregistrer le r\xE9f\xE9rencement")}</button></div></div>`);
          ...
          _push(`<div class="${ssrRenderClass(unref(actionStyles).bottomSave)}"><button type="button"${ssrIncludeBooleanAttr(unref(saving) === "seo") ? " disabled" : ""}>${ssrInterpolate(unref(saving) === "seo" ? "Enregistrement\u2026" : "Enregistrer le r\xE9f\xE9rencement")}</button></div></section>`);
        } else if (unref(activeTab) === "products") {
          _push(`<section id="panel-products" class="${ssrRenderClass(unref(styles).adminPanel)}" role="tabpanel" aria-labelledby="tab-products"><div class="${ssrRenderClass([unref(styles).panelTitle, unref(actionStyles).responsivePanelTitle])}"><div><small>CATALOGUE</small><h2>Produits</h2><p>Cr\xE9ez, modifiez ou masquez les articles de la boutique.</p></div><button data-demo-interactive>+ Nouveau produit</button></div>`);
          if (unref(products).length) {
            ...
            ssrRenderList(unref(products), (p) => {
              ...
              _push(`<article>`);
              if (p.image) {
                _push(`<img${ssrRenderAttr("src", p.image.content + `?size=${p.image.width}x${(_a2 = p.image) == null ? void 0 : _a2.height}`)}${ssrRenderAttr("alt", p.name)}${ssrRenderAttr("width", p.image.width)}${ssrRenderAttr("height", p.image.height)}>`);
              } else {
                _push(`<!---->`);
              }
              _push(`<div><h3>${ssrInterpolate(p.name)}</h3><span>${ssrInterpolate((p.priceCents / 100).toFixed(2))} \u20AC \xB7 ${ssrInterpolate(p.active ? "En ligne" : "Masqu\xE9")}</span></div><button data-demo-interactive>Modifier</button><button>Supprimer</button></article>`);
            });
            _push(`<!--]--></div>`);
          } else {
          ...
          _push(`<section id="panel-promotions" class="${ssrRenderClass(unref(styles).adminPanel)}" role="tabpanel" aria-labelledby="tab-promotions"><div class="${ssrRenderClass([unref(styles).panelTitle, unref(actionStyles).responsivePanelTitle])}"><div><small>R\xC9DUCTIONS ET CODES PROMO</small><h2>Promotions</h2><p>G\xE9rez les r\xE9ductions automatiques et les codes promo applicables au catalogue.</p></div></div><div class="${ssrRenderClass([unref(styles).panelTitle, unref(actionStyles).responsivePanelTitle])}" style="${ssrRenderStyle({ "margin-top": "24px" })}"><div><small>R\xC9DUCTIONS AUTOMATIQUES</small><h2>R\xE9ductions</h2></div><button type="button" data-demo-interactive>+ Nouvelle r\xE9duction</button></div>`);
          if (unref(discounts).length) {
            ...
            ssrRenderList(unref(discounts), (d) => {
              ...
              ssrRenderList(d.rules.filter((r) => r.scope === "product").slice(0, 1), (r) => {
                var _a2;
                _push(`<!--[-->`);
                if ((_a2 = unref(products).find((p) => p.id === r.targetId)) == null ? void 0 : _a2.image) {
              ...
              _push(`<!--]--><div><strong>${ssrInterpolate(d.label)}</strong><small>${ssrInterpolate(d.type === "percent" ? `${d.value} %` : `${(d.value / 100).toFixed(2)} \u20AC`)} \xB7 ${ssrInterpolate(d.active ? "Active" : "Inactive")}</small>`);
              ...
              _push(`<small>${ssrInterpolate(d.rules.map((r) => scopeLabel(r.scope, r.targetId)).join(" \xB7 "))}</small></div></div><button type="button" data-demo-interactive>Modifier</button><button type="button">Supprimer</button></article>`);
            });
            _push(`<!--]--></div>`);
          } else {
            _push(`<p class="${ssrRenderClass(unref(styles).adminEmpty)}">Aucune r\xE9duction configur\xE9e.</p>`);
          }
          ...
          if (unref(promoCodes).length) {
            ...
            ssrRenderList(unref(promoCodes), (p) => {
              _push(`<article><div><strong>${ssrInterpolate(p.code)}</strong><small>${ssrInterpolate(p.active ? "Actif" : "Inactif")}${ssrInterpolate(p.startsAt || p.endsAt ? ` \xB7 ${p.startsAt ? `Du ${p.startsAt.slice(0, 10)}` : ""}${p.endsAt ? ` au ${p.endsAt.slice(0, 10)}` : ""}` : "")}</small><small>${ssrInterpolate(p.rules.map((r) => `${scopeLabel(r.scope, r.targetId)} \u2014 ${r.type === "percent" ? `${r.value} %` : `${(r.value / 100).toFixed(2)} \u20AC`}`).join(" \xB7 "))}</small></div><button type="button" data-demo-interactive>Modifier</button><button type="button">Supprimer</button></article>`);
            });
            _push(`<!--]--></div>`);
          } else {
            _push(`<p class="${ssrRenderClass(unref(styles).adminEmpty)}">Aucun code promo configur\xE9.</p>`);
          }
          if (unref(discountDraft)) {
            ...
            }, ssrGetDirectiveProps(_ctx, vDemoReadonly, unref(isDemo))))}><button type="button" data-demo-interactive aria-label="Fermer">\xD7</button><h2>${ssrInterpolate(unref(discountDraft).id ? "Modifier" : "Cr\xE9er")} une r\xE9duction</h2><label>Libell\xE9<input${ssrRenderAttr("value", unref(discountDraft).label)} required placeholder="Ex : Soldes \xE9t\xE9"></label><label>Type <select><option value="percent"${ssrIncludeBooleanAttr(Array.isArray(unref(discountDraft).type) ? ssrLooseContain(unref(discountDraft).type, "percent") : ssrLooseEqual(unref(discountDraft).type, "percent")) ? " selected" : ""}>Pourcentage (%)</option><option value="fixed"${ssrIncludeBooleanAttr(Array.isArray(unref(discountDraft).type) ? ssrLooseContain(unref(discountDraft).type, "fixed") : ssrLooseEqual(unref(discountDraft).type, "fixed")) ? " selected" : ""}>Montant fixe (\u20AC)</option></select></label><label>Valeur <div class="${ssrRenderClass(unref(styles).inputSuffix)}"><input${ssrRenderAttr("value", unref(discountValueEuros))} type="number" min="0" step="0.01"${ssrRenderAttr("max", unref(discountDraft).type === "percent" ? 100 : 9999.99)} required><span>${ssrInterpolate(unref(discountDraft).type === "percent" ? "%" : "\u20AC")}</span></div></label><div class="${ssrRenderClass(unref(styles).checks)}"><label><input${ssrIncludeBooleanAttr(Array.isArray(unref(discountDraft).active) ? ssrLooseContain(unref(discountDraft).active, null) : unref(discountDraft).active) ? " checked" : ""} type="checkbox"> Active</label></div><label>Date de d\xE9but (optionnel)<input${ssrRenderAttr("value", unref(discountDraft).startsAt)} type="date"></label><label>Date de fin (optionnel)<input${ssrRenderAttr("value", unref(discountDraft).endsAt)} type="date"></label><fieldset><legend>R\xE8gles d&#39;application</legend><!--[-->`);
            ssrRenderList(unref(discountDraft).rules, (rule, index) => {
              _push(`<div class="${ssrRenderClass(unref(actionStyles).titleActions)}" style="${ssrRenderStyle({ "flex-wrap": "wrap", "margin-bottom": "8px" })}"><select${ssrRenderAttr("aria-label", `Scope r\xE8gle ${index + 1}`)}><option value="product"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "product") : ssrLooseEqual(rule.scope, "product")) ? " selected" : ""}>Produit</option><option value="category"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "category") : ssrLooseEqual(rule.scope, "category")) ? " selected" : ""}>Cat\xE9gorie</option><option value="universe"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "universe") : ssrLooseEqual(rule.scope, "universe")) ? " selected" : ""}>Univers</option></select>`);
              if (rule.scope === "product") {
                _push(`<select${ssrRenderAttr("aria-label", `Produit r\xE8gle ${index + 1}`)}><!--[-->`);
                ...
                _push(`<select${ssrRenderAttr("aria-label", `Cat\xE9gorie r\xE8gle ${index + 1}`)}><!--[-->`);
                ...
                _push(`<select${ssrRenderAttr("aria-label", `Univers r\xE8gle ${index + 1}`)}><!--[-->`);
            ...
            _push(`<!--]--><button type="button">+ Ajouter une r\xE8gle</button></fieldset><div class="${ssrRenderClass(unref(actionStyles).actions)}"><button type="button">Annuler</button><button type="submit"${ssrIncludeBooleanAttr(unref(promotionSaving)) ? " disabled" : ""}>${ssrInterpolate(unref(promotionSaving) ? "Enregistrement\u2026" : "Enregistrer")}</button></div></form></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(promoCodeDraft)) {
            _push(`<div class="${ssrRenderClass(unref(styles).modal)}"><form${ssrRenderAttrs(mergeProps({
              class: unref(styles).editor
            }, ssrGetDirectiveProps(_ctx, vDemoReadonly, unref(isDemo))))}><button type="button" data-demo-interactive aria-label="Fermer">\xD7</button><h2>${ssrInterpolate(unref(promoCodeDraft).id ? "Modifier" : "Cr\xE9er")} un code promo</h2><label>Code<input${ssrRenderAttr("value", unref(promoCodeDraft).code)} required placeholder="Ex : ETE2025" style="${ssrRenderStyle({ "text-transform": "uppercase" })}"></label><div class="${ssrRenderClass(unref(styles).checks)}"><label><input${ssrIncludeBooleanAttr(Array.isArray(unref(promoCodeDraft).active) ? ssrLooseContain(unref(promoCodeDraft).active, null) : unref(promoCodeDraft).active) ? " checked" : ""} type="checkbox"> Actif</label></div><label>Date de d\xE9but (optionnel)<input${ssrRenderAttr("value", unref(promoCodeDraft).startsAt)} type="date"></label><label>Date de fin (optionnel)<input${ssrRenderAttr("value", unref(promoCodeDraft).endsAt)} type="date"></label><fieldset><legend>R\xE8gles de r\xE9duction</legend><!--[-->`);
            ssrRenderList(unref(promoCodeDraft).rules, (rule, index) => {
              _push(`<div class="${ssrRenderClass(unref(actionStyles).titleActions)}" style="${ssrRenderStyle({ "flex-wrap": "wrap", "margin-bottom": "8px" })}"><select${ssrRenderAttr("aria-label", `Scope r\xE8gle ${index + 1}`)}><option value="all"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "all") : ssrLooseEqual(rule.scope, "all")) ? " selected" : ""}>Tout le catalogue</option><option value="product"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "product") : ssrLooseEqual(rule.scope, "product")) ? " selected" : ""}>Produit</option><option value="category"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "category") : ssrLooseEqual(rule.scope, "category")) ? " selected" : ""}>Cat\xE9gorie</option><option value="universe"${ssrIncludeBooleanAttr(Array.isArray(rule.scope) ? ssrLooseContain(rule.scope, "universe") : ssrLooseEqual(rule.scope, "universe")) ? " selected" : ""}>Univers</option></select>`);
              if (rule.scope === "product") {
                _push(`<select${ssrRenderAttr("aria-label", `Produit r\xE8gle ${index + 1}`)}><!--[-->`);
                ssrRenderList(unref(products), (prod) => {
                  _push(`<option${ssrRenderAttr("value", prod.id)}${ssrIncludeBooleanAttr(Array.isArray(rule.targetId) ? ssrLooseContain(rule.targetId, prod.id) : ssrLooseEqual(rule.targetId, prod.id)) ? " selected" : ""}>${ssrInterpolate(prod.name)}</option>`);
              ...
              _push(`<select${ssrRenderAttr("aria-label", `Type r\xE8gle ${index + 1}`)}><option value="percent"${ssrIncludeBooleanAttr(Array.isArray(rule.type) ? ssrLooseContain(rule.type, "percent") : ssrLooseEqual(rule.type, "percent")) ? " selected" : ""}>%</option><option value="fixed"${ssrIncludeBooleanAttr(Array.isArray(rule.type) ? ssrLooseContain(rule.type, "fixed") : ssrLooseEqual(rule.type, "fixed")) ? " selected" : ""}>\u20AC fixe</option></select><div class="${ssrRenderClass(unref(styles).inputSuffix)}"><input${ssrRenderAttr("value", rule.value)} type="number" min="0"${ssrRenderAttr("max", rule.type === "percent" ? 100 : 999999)}${ssrRenderAttr("aria-label", `Valeur r\xE8gle ${index + 1}`)}><span>${ssrInterpolate(rule.type === "percent" ? "%" : "\u20AC")}</span></div><button type="button"${ssrIncludeBooleanAttr(unref(promoCodeDraft).rules.length <= 1) ? " disabled" : ""}>\u2212 </button></div>`);
            });
            _push(`<!--]--><button type="button">+ Ajouter une r\xE8gle</button></fieldset><div class="${ssrRenderClass(unref(actionStyles).actions)}"><button type="button">Annuler</button><button type="submit"${ssrIncludeBooleanAttr(unref(promotionSaving)) ? " disabled" : ""}>${ssrInterpolate(unref(promotionSaving) ? "Enregistrement\u2026" : "Enregistrer")}</button></div></form></div>`);
          } else {
            _push(`<!---->`);
          ...
          _push(`<section id="panel-users" class="${ssrRenderClass(unref(styles).adminPanel)}" role="tabpanel" aria-labelledby="tab-users"><div class="${ssrRenderClass([unref(styles).panelHeading, unref(actionStyles).usersHeading])}"><div><small>ACC\xC8S ET DROITS</small><h2>Utilisateurs</h2><p>G\xE9rez les r\xF4les administrateur et l\u2019activation des comptes.</p></div><button type="button" class="${ssrRenderClass(unref(actionStyles).secondaryButton)}"${ssrIncludeBooleanAttr(unref(creatingDemo)) ? " disabled" : ""}>${ssrInterpolate(unref(creatingDemo) ? "Cr\xE9ation\u2026" : "Cr\xE9er un utilisateur temporaire de d\xE9mo")}</button></div>`);
          if (unref(demoCredentials2)) {
            _push(`<div class="${ssrRenderClass(unref(actionStyles).demoCredentials)}"><strong>Identifiants temporaires \xE0 transmettre</strong><span class="${ssrRenderClass(unref(actionStyles).demoCredentialLine)}"><b>E-mail :</b><code>${ssrInterpolate(unref(demoCredentials2).email)}</code><button type="button" class="${ssrRenderClass([unref(actionStyles).copyButton, unref(copiedDemoField) === "email" && unref(actionStyles).copied])}" aria-label="Copier l\u2019e-mail"${ssrRenderAttr("title", unref(copiedDemoField) === "email" ? "E-mail copi\xE9" : "Copier l\u2019e-mail")}><img${ssrRenderAttr("src", unref(copyIcon))} alt=""></button></span><span class="${ssrRenderClass(unref(actionStyles).demoCredentialLine)}"><b>Mot de passe :</b><code>${ssrInterpolate(unref(demoCredentials2).password)}</code><button type="button" class="${ssrRenderClass([unref(actionStyles).copyButton, unref(copiedDemoField) === "password" && unref(actionStyles).copied])}" aria-label="Copier le mot de passe"${ssrRenderAttr("title", unref(copiedDemoField) === "password" ? "Mot de passe copi\xE9" : "Copier le mot de passe")}><img${ssrRenderAttr("src", unref(copyIcon))} alt=""></button></span><small>Le mot de passe n\u2019est affich\xE9 qu\u2019apr\xE8s la cr\xE9ation de ce compte.</small></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(users).length) {
            ...
            ssrRenderList(unref(users), (user) => {
              _push(`<article><div><strong>${ssrInterpolate(user.email)}</strong><small>Cr\xE9\xE9 le ${ssrInterpolate(new Date(user.createdAt).toLocaleDateString("fr-FR"))}</small></div><select${ssrRenderAttr("aria-label", `R\xF4le de ${user.email}`)}><option value="customer"${ssrIncludeBooleanAttr(Array.isArray(user.role) ? ssrLooseContain(user.role, "customer") : ssrLooseEqual(user.role, "customer")) ? " selected" : ""}>Client</option><option value="admin"${ssrIncludeBooleanAttr(Array.isArray(user.role) ? ssrLooseContain(user.role, "admin") : ssrLooseEqual(user.role, "admin")) ? " selected" : ""}>Administrateur</option><option value="demo"${ssrIncludeBooleanAttr(Array.isArray(user.role) ? ssrLooseContain(user.role, "demo") : ssrLooseEqual(user.role, "demo")) ? " selected" : ""}>D\xE9monstration \u2014 lecture seule</option></select><label><input${ssrIncludeBooleanAttr(Array.isArray(user.active) ? ssrLooseContain(user.active, null) : user.active) ? " checked" : ""} type="checkbox"> Actif</label>`);
              if (user.role !== "demo") {
                _push(`<button type="button" class="${ssrRenderClass(unref(actionStyles).resetPassword)}"${ssrIncludeBooleanAttr(unref(resettingUserId) === user.id) ? " disabled" : ""}>${ssrInterpolate(unref(resettingUserId) === user.id ? "Envoi\u2026" : "R\xE9initialiser le mot de passe")}</button>`);
              } else {
                _push(`<!---->`);
              }
              if (user.role === "demo") {
      ...
      if (unref(draft)) {
        _push(`<div class="${ssrRenderClass(unref(styles).modal)}"><form${ssrRenderAttrs(mergeProps({
          class: unref(styles).editor
        }, ssrGetDirectiveProps(_ctx, vDemoReadonly, unref(isDemo))))}><button type="button" data-demo-interactive aria-label="Fermer">\xD7</button><h2>${ssrInterpolate(unref(draft).id ? "Modifier" : "Cr\xE9er")} un produit</h2><label>Nom<input${ssrRenderAttr("value", unref(draft).name)} required>`);
        _push(ssrRenderComponent(_component_FieldValidation, {
        ...
        _push(`</label><label>Cat\xE9gories<select multiple required><!--[-->`);
        ...
        _push(`<!--]--></select><small>Maintenez Ctrl ou Cmd pour s\xE9lectionner plusieurs cat\xE9gories.</small>`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: unref(validation).product,
        ...
        _push(`<!--]--></select><small>Maintenez Ctrl ou Cmd pour s\xE9lectionner plusieurs univers.</small>`);
        _push(ssrRenderComponent(_component_FieldValidation, {
          issues: unref(validation).product,
...
//# sourceMappingURL=admin-CRC7E_fA.mjs.map
